(function () {
  ...
  function initializeEngine() {
    ...
    message.innerHTML = "What You Seek Is Seeking You👀     --Rumi";
    ...
    messageContainer.style.left =
      window.innerWidth / 2 - messageContainer.offsetWidth / 2 + "px";
    messageContainer.style.top =
      window.innerHeight / 2 - messageContainer.offsetHeight / 2 + "px";
  ...
  function initializeScene() {
    miniMap = new Demonixis.Gui.MiniMap(
      map[0].length,
      map.length,
      "canvasContainer"
    );
  ...
  function moveCamera(direction, delta) {
    ...
    var moveParamaters = {
      translation:
        typeof delta != "undefined"
          ? delta.translation
          : cameraHelper.translation,
      rotation:
        typeof delta != "undefined" ? delta.rotation : cameraHelper.rotation,
    };

    switch (direction) {
      case "up":
        position.x -=
          Math.sin(-camera.rotation.y) * -moveParamaters.translation;
        position.z -= Math.cos(-camera.rotation.y) * moveParamaters.translation;
        document.getElementById("walkSound").play(); // Play walking sound
        break;
      case "down":
    ...
    var tx = Math.abs(
      Math.floor((cameraHelper.origin.x + camera.position.x * -1) / 100)
    );
    var ty = Math.abs(
      Math.floor((cameraHelper.origin.z + camera.position.z * -1) / 100)
    );

    // next position
    var newTx = Math.abs(
      Math.floor((cameraHelper.origin.x + position.x * -1 + offset) / 100)
    );
    var newTy = Math.abs(
      Math.floor((cameraHelper.origin.z + position.z * -1 + offset) / 100)
    );
